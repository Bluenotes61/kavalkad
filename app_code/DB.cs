...
namespace NFN {
  ...
  public class DB {
    ...
    public static CMS Cms {
      get { 
        try { return (CMS)HttpContext.Current.Session["CMS"]; }
        catch { return null; }
      }
    }
    ...
    private static bool IsValidSql(String sqlStr) {
      if ((Cms == null || !Cms.User.IsAdmin) && (
        sqlStr.ToLower().Contains("<script") || 
        sqlStr.ToLower().Contains("--") || 
        sqlStr.ToLower().Contains("+char(") || 
        sqlStr.ToLower().Contains("xp_cmdshell"))) 
      {
        Util.Debug("SQL Injection - " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " - " + sqlStr);
        return false;
      }
      else return true;
    }
    ...
    public static DataSet GetDS(String sqlStr, int startRow, int nOfRows, String table, String connStr) {
      if (!IsValidSql(sqlStr)) return null;
    ...
    public static String GetDateString(DataSet ds, int row, String col, bool withtime) {
      try {
        ...
        if (withtime) return dt.ToString(CMS.SiteSetting("dateTimeFormat"));
        else return dt.ToString(CMS.SiteSetting("dateFormat"));
      }
      catch {
        return "";
      }
