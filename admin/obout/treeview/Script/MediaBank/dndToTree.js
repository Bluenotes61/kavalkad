...
function ob_dnd_t11(event)
{
  ...
  if(ob_dnd_tvu==null)
  {
    ...
    img.src = ob_dnd_tvi.firstChild.firstChild.src;
    ...
    etdContent.innerHTML=ob_dnd_tvi.firstChild.nextSibling.innerHTML;
    ob_dnd_tvu.appendChild(tbl);*/

    dragdiv = document.createElement("DIV");
    dragdiv.id = ob_dnd_tvi.id;
    var imgdiv = dragdiv.appendChild(document.createElement("div"));
    var img = imgdiv.appendChild(document.createElement("img"));
    img.src = ob_dnd_tvi.firstChild.firstChild.src;
    ob_dnd_tvu.appendChild(dragdiv);
  ...
  {
    var event=window.event;
    ob_dnd_tvu.style.left = String(event.x+ob_dnd_tvt-5) + "px";
    ob_dnd_tvu.style.top = String(event.y+ob_dnd_tvq-5) + "px";

    // if copying
    if (notesDivCopy && ob_dnd_allowCopy)
    {
      // move the copy div at the mouse coords
      notesDivCopy.style.left = String(event.x + ob_dnd_tvt - 12) + "px";
      notesDivCopy.style.top = String(event.y + ob_dnd_tvq + 12) + "px";
    }
  }
  else
  {
    ob_dnd_tvu.style.left = String(event.pageX-5) + "px";
    ob_dnd_tvu.style.top = String(event.pageY-5) + "px";

    // if copying
    if (notesDivCopy && ob_dnd_allowCopy)
    {
      // move the copy div at the mouse coords
      notesDivCopy.style.left = String(event.pageX - 12) + "px";
      notesDivCopy.style.top = String(event.pageY + 12) + "px";
    }
  }
    ...
    if(ob_dnd_tvf.parentNode.tagName=="DIV")
    {
      ...
      if(ob_dnd_tvf.firstChild.firstChild.childNodes.length==3)
      {
        ...
        if(ob_dnd_tvf.firstChild.firstChild.firstChild.firstChild.tagName=="IMG")
        {
          ...
          if((s=="ik.gif")||(s=="hr.gif")||(s=="_l.gif")||(s=="us.gif")){}
...
function NotesKeyModifierWatch(event)
{
  ...
  if (notesDivCopy == null)
  {
    ...
    notesDivCopy.innerHTML = '<img src="TreeIcons/Icons/plus.gif">';
...
function ob_dnd_handleCopy (ob_drag_item, ob_drag_destination) {
  var destid = ob_drag_destination.firstChild.firstChild.firstChild.nextSibling.nextSibling.id;
  var mess = db.docCopiedExternal(ob_drag_item.id, destid);
...
function ob_dnd_handleMove (ob_drag_item, ob_drag_destination) {
  var destid = ob_drag_destination.firstChild.firstChild.firstChild.nextSibling.nextSibling.id;
  var mess = db.docMovedExternal(ob_drag_item.id, destid);
  if (mess.length > 0) alert(mess);
  return (mess.length == 0);
}

/*function DocDroppedOutside(drag) {
  var trash = document.getElementById("trash");
  var trashpos = findPos(trash);
  var itempos = findPos(drag);
  var trashok = itempos[0] >= trashpos[0] && itempos[0] <= trashpos[0]+16 && itempos[1] >= trashpos[1] && itempos[1] <= trashpos[1]+16;
  if (trashok)
    DelDocument(drag.id.substring(6));
}
*/